function build_acopf_feas_II(_case,_p,_θ; Iea = [true for i in 1:length(_case.lines)])
    ## Requires Ipopt
    #################
    ...
    @constraint(m, flowP[b = _case.buses], sum( v[b] * (gl[l] * cos(δθ[l]) + bl[l] * sin(δθ[l]))  for l in _case.lines_at_bus[b] if Iea[l])  == _case.bus_Pd[b] - sum( _p[g] for g in _case.generators_at_bus[b] ) - ep[b] + em[b] )
